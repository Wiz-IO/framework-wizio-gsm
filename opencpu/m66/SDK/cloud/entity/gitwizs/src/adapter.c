...
s32 Adapter_DevSetQeng(Callback_ATResponse atRsp_callBack,void* userData)
{
    ...
    ret = Ql_RIL_SendATCmd("AT+QENG=1","AT+QENG=1",NULL,NULL,0);
    ...
    ret = Ql_RIL_SendATCmd("AT+QENG?","AT+QENG?",atRsp_callBack,userData,0);
...
//uh20 add mqtt socket init api
void Adapter_MQTT_Socket_Init(void)
{
    Hal_MQTT_Socket_Init();
}
...
{
    s32 gsm_status ,gprs_status;
    
    
RIL_NW_GetGSMState(&gsm_status);
    
     RIL_NW_GetGPRSState(&gprs_status);
