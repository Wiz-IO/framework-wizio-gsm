...
#define LCDIF_RSTB                  (*(volatile uint16_t *)(LCDIF_Base + 0x0010)) // Directly controls the LRSTB pin
#define LCDIF_RESET1                (1 << 0)
#define LCDIF_RESET0                (0 << 0)

#define LCDIF_SIF_PIX_CON_REG       (*(volatile uint32_t *)(LCDIF_Base + 0x0018))

// (LCD_SIF0_TIMING.x * (LCD_SIF_CON.SIF0_DIV2 + 1) ) + 1
#define LCDIF_SIF0_TIMING           (*(volatile uint32_t *)(LCDIF_Base + 0x001C))
#define LCDIF_WR_2ND(v)             (((v) & 0x000F) << 0)   // Second phase timing of LSCK when transfer
#define LCDIF_WR_1ST(v)             (((v) & 0x000F) << 4)   // First phase timing of LSCK when transfer
#define LCDIF_RD_2ND(v)             (((v) & 0x000F) << 8)
#define LCDIF_RD_1ST(v)             (((v) & 0x000F) << 12)
#define LCDIF_CSH(v)                (((v) & 0x000F) << 16)  // Chip selection hold time
#define LCDIF_CSS(v)                (((v) & 0x000F) << 20)  // Chip selection setup time
...
#define LCDIF_SIF0_SIZE8B           (0 << 0)    // Interface size of serial interface
...
#define LCDIF_SIF0_4WIRE            (0 << 3)    // Enables N-wire mode of serial interface
#define LCDIF_SIF0_3WIRE            (1 << 3)
#define LCDIF_SIF0_SDI              (1 << 4)    // Set to 1 to read data from LSDI pin
#define LCDIF_SIF0_1ST_POL          (1 << 5)    // First-phase polarity of LSCK for serial interface
#define LCDIF_SIF0_SCK_DEF          (1 << 6)    // Default of LSCK when no data are transferred, LSCK low/hi
#define LCDIF_SIF0_DIV2             (1 << 7)    // Slows down timing
...
#define LCDIF_SIF0_HW_CS            (1 << 24)   // CS control
#define LCDIF_SIF1_HW_CS            (1 << 28)

#define LCDIF_SIF_CS                (*(volatile uint32_t *)(LCDIF_Base + 0x002C)) // chip selection
#define LCDIF_CS0                   (1 << 0)    // chip selection level HW_1/SW_0
#define LCDIF_CS1                   (1 << 1)

#define LCDIF_PCNF0                 (*(volatile uint32_t *)(LCDIF_Base + 0x0030)) // parallel interface
...
#define LCDIF_PCNF1                 (*(volatile uint32_t *)(LCDIF_Base + 0x0034)) // parallel interface
#define LCDIF_WST(v)                (((v) & 0x003F) << 0)
#define LCDIF_C2WS(v)               (((v) & 0x000F) << 8)
#define LCDIF_C2WH(v)               (((v) & 0x000F) << 12)
#define LCDIF_RLT(v)                (((v) & 0x003F) << 16)
...
#define LCDIF_F_BGR                 (0 << 0)    // Sequence
#define LCDIF_F_RGB                 (1 << 0)
#define LCDIF_F_SIGNIF              (1 << 1)    // Significance
#define LCDIF_F_PADDLSB             (0 << 2)    // Padding bits
#define LCDIF_F_PADDMSB             (1 << 2)
#define LCDIF_F_RGB332              (0 << 3)    // Color format
#define LCDIF_F_RGB444              (1 << 3)
#define LCDIF_F_RGB565              (2 << 3)
#define LCDIF_F_RGB666              (3 << 3)
#define LCDIF_F_RGB888              (4 << 3)
#define LCDIF_F_ITF_8B              (0 << 6)    // Interface width
#define LCDIF_F_ITF_16B             (1 << 6)
#define LCDIF_F_ITF_9B              (2 << 6)
#define LCDIF_F_ITF_18B             (3 << 6)
#define LCDIF_COMMAND(v)            (((v) & 0x003F) << 8) // Number of commands to be sent
...
static inline uint16_t makeColor565(uint8_t r, uint8_t g, uint8_t b)
{
