...
static inline bool GPIO_DATAIN(uint8_t Pin)
{
    return (Pin < 0x20) ? ((GPIO_DIN0 & (1UL << Pin)) ? true : false) : (Pin < 0x40) ? ((GPIO_DIN1 & (1UL << (Pin - 0x20))) ? true : false) : -1;
}
    ...
    {                                                                                    \
        ...
        {                                                                                \
            ...
            if (p < 8)                                                                   \
                ...
                Mask = GPIO_MODE6MASK;                                                   \
            GPIO_MODE(RegIdx) = (GPIO_MODE(RegIdx) & Mask) | (((Mode)&Mask) << ModePos); \
        }                                                                                \
    } while (0)
